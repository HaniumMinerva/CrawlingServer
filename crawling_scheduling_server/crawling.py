...
def select_query_company(conn, mydict):
    """
    DB SELECT
    """
    try:
        with conn.cursor() as cursor:
            sql = "select company_name, company_id from Company;" # hoho 여기엔 테이블 이름
            cursor.execute(sql)
            for i in range(cursor.rowcount):
                company_name, company_id = cursor.fetchone()
                mydict[company_name] = company_id

    finally:
        pass

def insert_query(conn, recruit_name, company_id, recruitment_form, recruit_category, recruitment_school, work_location, pay, position, start_day, finish_day, require_documents, status, require_how_many):
    """
    DB INSERT
    """
    try:
        with conn.cursor() as cursor:
            sql = "insert into recruitment (recruit_name, company_id, recruitment_form, recruit_category, recruitment_school, work_location, pay, position, start_day, finish_day, require_documents, status, require_how_many) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            cursor.execute(sql, (recruit_name, company_id, recruitment_form, recruit_category, recruitment_school, work_location, pay, position, start_day, finish_day, require_documents, status, require_how_many))
        conn.commit()
...
def main():
    ...
    print("1111111111111")
    select_query_company(conn, company_dict)
    print("2222222222222")
    ...
    for i in range(0,len(links)):
        ...
        # recruit_name, company_id, company_occupation, company_category, recruitment_school, work_location, pay, position, start_day, finish_day, require_documents, status, require_how_many
        insert_query(conn, job_title, company_id, recruitment_form, recruitment_work, recruitment_school, work_location, pay, position, start_day, finish_day, require_documents, 0, recruitment_how_many)
